<template>
  <div>
    ...
    <el-table stripe :data="list" style="margin-top:15px;">
      ...
      <el-table-column prop="sysName" label="产品分类" align="left">
        <template slot-scope="scope">
          <el-button type="text" @click="editDetail(scope.row, 'show')">{{scope.row.name}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="isDisable" align="left" label="服务类型" width="160">
        <template slot-scope="scope">
...
<script>
import { getProductTypePage, ProductIsDisableType, deleteProductType } from '@/api/index'
...
export default {
  ...
  methods: {
    ...
    getList() {
      getProductTypePage(this.form).then((response) => {
    ...
    setDisable(row) {
      Confirm(`确定${!row.isDisable ? '禁用' : '启用'}该产品吗?`, () => {
        ProductIsDisableType({ id: row.id }).then(res => {
          if (res.code != 200) {
