...
export default {
  ...
  watch: {
    isIntegral(val) {
      if (this.isIntegral == 0) {//非积分业务过滤掉流程
        this.list = JSON.parse(JSON.stringify(this.list2)).filter(item => item.isIntegral != 1)
      } else {
        this.list = JSON.parse(JSON.stringify(this.list2))
      }
    },
    oilCouponChange(val) {
      if (this.isOilCoupon == 0) {//积分业务过滤掉确认下发及财务付款流程
        this.list = JSON.parse(JSON.stringify(this.list2)).filter(item => item.isOilCoupon == 0)
      } else {
        this.list = JSON.parse(JSON.stringify(this.list2))
      }
  ...
  methods: {
    Changeload(result = {}) {
      this.$nextTick(() => {
        if (result && result.generalDirectorId) {
          this.list.forEach((item, index) => {
            if (index == 0) {
              item.approvalName = result.generalDirectorName
            }
          })
          // this.$set(this.list[0], 'approvalName', result.generalDirectorName)
          // this.list[0].approvalName = result.generalDirectorName
        }
      })
    },
    ...
    async onload(result = {}) {
      ...
      await getApprovalProcessConfigList({ type: this.type }).then(res => {
        this.list = res.data || []
