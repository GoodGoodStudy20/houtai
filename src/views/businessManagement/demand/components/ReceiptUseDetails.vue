  ...
  methods: {
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (index === 2) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalList.map(item => item.amount || 0)))
        }
        if (index === 3) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalList.map(item => item.availableMoney || 0)))
        }
        if (index === 6) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalList.map(item => item.surplusAvailableMoney || 0)))
        }
        if (index === 5) {
          let a = this.returnMoneyApprovalList.map(item => item.availAbleMoneyRecordList.map(item1 => item1.usedMoney || 0))
          let c = []
          a.forEach((item, index) => {
            c.push(...item)
          })
          sums[index] = this.$options.filters.toThousands(NumberAdd(c) || 0)
        }
        // if (index === 5) {
