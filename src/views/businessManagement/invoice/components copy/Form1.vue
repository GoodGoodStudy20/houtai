<template>
  <el-form size="small" ref="form" :model="form" label-width="auto" :disabled="disabled" class="block-form">
    ...
    <el-form-item label="开票主体" prop="subjectListId" :rules="[{ required: true, message: '开票主体不可为空', trigger: 'blur' }]">
      ...
      <el-select v-model="form.subjectListId" placeholder="请选择" style="width:500px" :multiple="true" clearable filterable :disabled="!isDisabled">
...
export default {
  ...
  methods: {
    getSubjectList(params = {}) {
      getSubjectList({ ...params, type: this.form.type, customerId: this.form.customerId }).then(res => {
