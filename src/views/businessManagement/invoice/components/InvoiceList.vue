<template>
  <div>
    <div class="top" v-if="!select">
      ...
      <el-button type="primary" @click="editDetail('', 'add')" size="small" style="margin:10px 0" v-if="!select">新增</el-button>
    </div>
    <el-tabs v-model="activeName" @tab-click="handleClick" v-if="!select">
      <el-tab-pane label="全部发票" name="0"></el-tab-pane>
      <el-tab-pane label="作废记录" name="1">
        <span slot="label" :class="{invoice:isTovoid}">作废记录</span>
      </el-tab-pane>
      <el-tab-pane label="冲红记录" name="2">
        <span slot="label" :class="{invoice:isFlushRed}">冲红记录</span>
      </el-tab-pane>
    </el-tabs>
    <div class="search-box">
      <el-form inline size="small">
        ...
        <!-- <el-form-item label="回款状态" v-if="form.invoicingType == 0">
        ...
        <el-form-item label="开票性质">
          <el-select v-model="form.invoicingNature" placeholder="请选择" style="width:150px">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="(item,i) in InvoiceType" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item>
        <!-- <el-form-item label="开票类型">
          <el-select v-model="invoicingStatus" placeholder="请选择" style="width:150px">
            <el-option label="全部" value=""></el-option>
            <el-option v-for="(item,i) in invoicingListStatus" :key="item.value" :label="item.label" :value="item.value"></el-option>
          </el-select>
        </el-form-item> -->
        ...
        <el-form-item label=" ">
          ...
          <el-button type="primary" icon="el-icon-refresh-right" @click="handleResetChange">重置</el-button>
        </el-form-item>
      </el-form>
    </div>
    <el-table stripe :data="list" show-summary :summary-method="getSummaries" ref="table" @sort-change="sortChange" @selection-change="handleSelectionChange" row-key="id" style="max-width: 100%;margin-top:15px;">
      ...
      <el-table-column label="开票主体" align="left" show-overflow-tooltip width="100px">
        <template slot-scope="scope">
      ...
      <el-table-column label="发票类型" align="left" v-if="!select">
        <template slot-scope="scope">
          {{InvoiceTypeList[scope.row.type]?InvoiceTypeList[scope.row.type].label:''}}
        </template>
      </el-table-column>
      <el-table-column label="开票性质" align="left">
        <template slot-scope="scope">
          {{InvoiceType[scope.row.invoicingNature]?InvoiceType[scope.row.invoicingNature].label:''}}
        </template>
      </el-table-column>
      <el-table-column label="开票类型" align="left" v-if="!select">
        <template slot-scope="scope">
          {{InvoicingType[scope.row.invoicingType]?InvoicingType[scope.row.invoicingType].label:''}}
        </template>
      </el-table-column>
      <el-table-column label="开票状态" width="70" align="left" v-if="!select">
        <template slot-scope="scope">
          <el-tag size="mini" v-if="scope.row.invoicingStatus==0" type="primary" disable-transitions>正常</el-tag>
          <el-tag size="mini" v-if="scope.row.invoicingStatus==1" type="warning" disable-transitions>部分作废</el-tag>
          <el-tag size="mini" class="tored" v-if="scope.row.invoicingStatus==2" type="warning" disable-transitions>部分冲红</el-tag>
          <el-tag size="mini" v-if="scope.row.invoicingStatus==3" type="warning" disable-transitions>作废</el-tag>
          <el-tag size="mini" class="tored" v-if="scope.row.invoicingStatus==4" type="warning" disable-transitions>冲红</el-tag>

          <!-- {{scope.row.invoicingStatus==0?'正常':'作废'}} -->
        </template>
      </el-table-column>
      <el-table-column label="作废份数" align="left" v-if="form.invoicingType==1">
        <template slot-scope="scope">
          {{scope.row.invoiceFileVoList && scope.row.invoiceFileVoList.length}}
        </template>
      </el-table-column>
      <el-table-column label="作废金额" align="right" v-if="form.invoicingType==1">
        <template slot-scope="scope">
          {{scope.row.invoicingAmount || 0 | toThousands}}
        </template>
      </el-table-column>
      <el-table-column label="冲红金额" align="right" v-if="form.invoicingType==2">
        <template slot-scope="scope">
          {{scope.row.invoicingAmount}}
        </template>
      </el-table-column>
      <!-- <el-table-column label="税率" prop="taxRate" align="center" width="80px"></el-table-column> -->
      <el-table-column label="申请金额" prop="invoicingAmount" align="right" min-width="120px" v-if="form.invoicingType==0">
        <template slot-scope="scope">
          <span v-if="scope.row.invoicingType==0">{{scope.row.invoicingAmount || 0 | toThousands}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column label="开票金额" prop="invoicingAmount" align="right" min-width="120px" v-if="form.invoicingType==0">
        <template slot-scope="scope">
          <span v-if="scope.row.invoicingType==0">{{scope.row.invoiceFileAmount  || 0 | toThousands}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>

      <el-table-column label="已回款" prop="returnAmount" min-width="120px" align="right" v-if="form.invoicingType==0">
        <template slot-scope="scope">
          <span v-if="scope.row.invoicingType==0">{{scope.row.returnAmount || 0 | toThousands}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>
      <el-table-column label="未回款" min-width="120px" prop="surplusAmount" align="right" v-if="form.invoicingType==0">
        <template slot-scope="scope">
          <span v-if="scope.row.invoicingType==0">{{scope.row.surplusAmount || 0 | toThousands}}</span>
          <span v-else>--</span>
        </template>
      </el-table-column>

      <!-- <el-table-column label="开票日期" prop="invoicingDate" align="left" sortable="custom" width="120" v-if="form.type==0"></el-table-column> -->
      <el-table-column label="作废日期" prop="voidDate" align="left" sortable="custom" width="120" v-if="form.invoicingType==1"></el-table-column>
      <!-- <el-table-column label="开票人" prop="invoicingUserName" align="left"></el-table-column> -->
      <el-table-column label="当前状态" align="left" width="120" v-if="!select">
        <template slot-scope="scope">
      ...
      <el-table-column label="开票时间" prop="invoicingDate" v-if="select" align="left" width="120"></el-table-column>
      <el-table-column label="提交人" prop="createByName" align="left" v-if="!select"></el-table-column>
      <el-table-column prop="submitTime" align="left" label="提交时间" width="160" v-if="!select"></el-table-column>
      <el-table-column align="left" label="操作" width="190" v-if="!select" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="text" style="color:red" @click="editDetail(scope.row.id,'flushRed')" v-if="scope.row.approvalStatus==2 && scope.row.invoicingType==0 && scope.row.createBy==user.id && ( scope.row.invoicingStatus!=4 && scope.row.invoicingStatus!=3)">冲红</el-button>
          <el-button type="text" style="color:red" @click="editDetail(scope.row.id,'tovoid')" v-if="scope.row.approvalStatus==2 && scope.row.invoicingType==0 && (scope.row.type==0 || scope.row.type==1) && ( scope.row.invoicingStatus!=3 && scope.row.invoicingStatus!=4) && scope.row.createBy==user.id">作废</el-button>
          <!-- <el-button type="text" style="color:red" v-if="scope.row.invoicingStatus==1">已作废</el-button> -->
        </template>
      </el-table-column>
    ...
    <div style="display:flex;justify-content:center">
      ...
      <div style="margin-top:10px" v-show="isShowExport && !select">
        <el-select size="small" v-model="exportType" placeholder="请选择" style="width:150px">
          <el-option label="导出列表" value="1"></el-option>
          <el-option label="导出列表明细" value="2"></el-option>
        </el-select>
      </div>
...
import { deleteInvoiceApproval, getInvoiceApprovalPage, updateStatusInvoiceApproval, revokeInvoiceApproval, submitInvoiceApproval, getCustomerList, getApprovalData } from '@/api/index.js'
...
export default {
  ...
  methods: {
    ...
    getApprovalData() {
      getApprovalData().then(res => {
        let resData = res.data
        if (resData.toVoidInvoice) {
          this.isTovoid = true
        }
        if (resData.negativeInvoice) {
          this.isFlushRed = true
        }
      })
    },
    handleClick() {
      let arr = [0, 1, 2]
      this.form.invoicingType = arr[this.activeName]
    ...
    exportClick() {
      ...
      if (this.exportType == 1) {
        url = '/cii-system/invoiceApproval/exportInvoiceApprovalListExcel'
      }
      if (this.exportType == 2) {
        url = '/cii-system/invoiceApproval/exportInvoiceApprovalInfoExcel'
      }
      newExportsExcel(url, param).then(res => {
        // newExportsExcel('/cii-system/invoiceApproval/exportInvoiceApprovalInfoExcel', param).then(res => {
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (this.select) {
          ...
          if (index === 5) {
            sums[index] = this.$options.filters.toThousands(this.invoicedata.totalInvoiceAmount || 0)
          }
          if (index === 6) {
            sums[index] = this.$options.filters.toThousands(this.invoicedata.totalReturnAmount || 0)
          }
          if (index === 7) {
            sums[index] = this.$options.filters.toThousands(this.invoicedata.totalSurplusAmount || 0)
          }
        }
        else {
          if (index === 3) {
          ...
          if (this.form.invoicingType == 0) {

            if (index === 9) {
              sums[index] = this.$options.filters.toThousands(this.invoicedata.totalInvoiceAmount || 0)
            }
            if (index === 10) {
              sums[index] = this.$options.filters.toThousands(this.invoicedata.totalInvoiceFileAmount || 0)
            }
            if (index === 11) {
              sums[index] = this.$options.filters.toThousands(this.invoicedata.totalReturnAmount || 0)
            }
          ...
          } else if (this.form.invoicingType == 1) {
            if (index === 10) {
              sums[index] = this.$options.filters.toThousands(this.invoicedata.totalInvoiceAmount || 0)
            }
          } else {
    ...
    getList() {
      getInvoiceApprovalPage({ ...this.form, ...this.params }).then(res => {
        ...
        this.list.forEach(item => {
          item.subject = []
          item.subjectName = []
          item.subjectVoList.forEach(item1 => {
            item.subject.push(item1.abbreviated)
            item.subjectName.push(item1.name)
          })
          item.subject = String(item.subject)
          item.subjectName = String(item.subjectName)
          // if (item.approvalStatus == 1 && item.invoicingType == 1) {
          //   this.isTovoid = true
          // }
          // if (item.approvalStatus == 1 && item.invoicingType == 2) {
          //   this.isFlushRed = true
          // }
        })
