      ...
      <el-form size="small" :disabled="disabled" inline :model="item" v-if="item.subjectId || editType === 'approve'">
        ...
        <div v-for="(item1, index1) in item.invoiceFileVoList" :key="index1">
          ...
          <el-form-item label="价税合计" required>
            <el-input v-model="item1.priceTaxTotal" placeholder="请输入价税合计" @change="priceTaxTotalChange" style="width: 140px" oninput="value=value.replace(/[^\d.]/g,'').replace(/^(\-)*(\d+)\.(\d\d).*$/,'$1$2.$3')"></el-input>
          </el-form-item>
          <el-form-item label="" required>
            <UploadFile v-model="item1.invoiceFile" :disabled="disabled" :limit="1" fileName="发票"></UploadFile>
          </el-form-item>
          <el-form-item label=" " style="width: 70px">
            <el-button type="primary" @click="addOneInvoiceFile(i, index1)" v-if="
                index1 == item.invoiceFileVoList.length - 1 &&
                editType != 'show'
              ">增加</el-button>
          </el-form-item>
          <el-form-item>
            <el-button type="danger" @click="delOneInvoiceFile(i, index1)" v-if="item.invoiceFileVoList.length > 1 && editType != 'show'">删除</el-button>
          </el-form-item>
        </div>
...
export default {
  ...
  methods: {
    ...
    commit() {
      ...
      let isCommit = true;
      this.list.forEach((item) => {
        if (item.subjectId != this.subjectListId) {
          this.$message.error('请选择正确的开票主体')
          isCommit = false
          return
        }
        item.invoiceFileVoList.forEach((it) => {
