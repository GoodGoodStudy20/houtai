<template>
  <el-dialog :visible.sync="value" title="回单使用详情" :before-close="dialogClose" :close-on-click-modal="false" center width="80%" :lock-scroll="false" :modal-append-to-body="false">
    <el-table stripe show-summary border sum-text='合计' :summary-method="getSummaries" :data="returnMoneyApprovalVoList" max-height="500px" style="max-width: 100%;margin-top:15px;" :header-cell-style="{'background-color':'#F4F8FF'}">
...
export default {
  ...
  props: ['isExportRecord', 'value', 'returnMoneyApprovalVoList'],
  data() {
    return {
      dialogInfo: false,
      dialogVisible: false
  ...
  created() {
    if (this.returnMoneyApprovalVoList && this.returnMoneyApprovalVoList.length > 0) {
      this.returnMoneyApprovalVoList.forEach(item => {
  ...
  methods: {
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        ...
        if (index === 2) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalVoList.map(item => item.amount)))
        }
        if (index === 3) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalVoList.map(item => item.availableMoney)))
        }
        if (index === 6) {
          sums[index] = this.$options.filters.toThousands(NumberAdd(this.returnMoneyApprovalVoList.map(item => item.surplusAvailableMoney)))
        }
        if (index === 5) {
          let a = this.returnMoneyApprovalVoList.map(item => item.availAbleMoneyRecordList.map(item1 => item1.usedMoney))
          let c = []
        ...
        // if (index === 5) {
        //   let a = this.returnMoneyApprovalVoList.map(item => item.availAbleMoneyRecordList.map(item1 => item1.surplusAvailableMoney))
        //   let c = []
        //   a.forEach((item, index) => {
        //     c.push(...item)
        //   })
