<template>
  <div>
    <div class="top-title">
      <div class="title-item table-text" v-if="type=='fromRemind'" @click="cancel"><img src="@/assets/img/left.png" alt=""><span>返回</span></div>
      <div v-else class="title-item table-text" v-for="item in path3" :key="item.id" @click="toClick(item)"><img src="@/assets/img/left.png" alt=""><span>{{item.name}}</span></div>

    </div>
    <div class="title-receivable">{{abbreviated}}-{{isActive==1?'逾期':'本月到期'}}应收详情</div>
    <el-table stripe :data="list" show-summary :summary-method="getSummaries" :span-method="objectSpanMethod" :header-cell-style="{ 'background-color': '#F4F8FF' }" style="max-width: 100%;margin-top:15px;">
      ...
      <el-table-column label="机构" min-width="160" prop="customerName" align="left">
      ...
      <el-table-column label="应收总额" prop="totalReceivable" align="right" min-width="120">
        <template slot-scope="scope">
      ...
      <el-table-column label="到期日" min-width="100" align="left">
        <template slot-scope="scope">
...
<script>
import { getBeOverdueReceivableInfo } from '@/api/index.js'
...
export default {
  ...
  created() {
    if (sessionStorage.getItem('path3')) {
      this.path3 = JSON.parse(sessionStorage.getItem('path3'))
    }
    ...
    if (this.$route.query.abbreviated) {
      this.abbreviated = this.$route.query.abbreviated
    }
  ...
  methods: {
    ...
    getListDataForRowAndColumn() {
      ...
      for (var i = 0; i < data.length; i++) {
        if (i === 0) {
          ...
          if (data[i].customerName === data[i - 1].customerName && data[i].channelName === data[i - 1].channelName) {
            // 如果typeDesc相等就累加，并且push 0
    ...
    getList() {
      ...
      getBeOverdueReceivableInfo(data).then(res => {
